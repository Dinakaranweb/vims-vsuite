...
class TicketController extends Controller
{
    ...
    public function storeTicket(Request $request){
        ...
        if($approval_status){
            ...
            $email_to = User::where('department', $ticket_to)->where('role', 'HOD')->where('is_active', 1)->first();

            $mail_details['content'] = 'You have received a Ticket - <b>'.$title.'</b> from <b>'.$ticket_from.'</b>';
            $mail_details['url'] = URL::to('admin/view/ticket/'.$task_id);
            
            $mail_details['title'] = 'Ticket Received';
            
            $subject = 'Ticket - '.$title.' from '.$ticket_from.' Received!';
            ...
            if($ticket_from == 'VC' || $ticket_from == 'Pro-VC' || $ticket_from == 'Registrar'){
                
                $email_to = User::where('department', $ticket_from)->where('role', 'SuperAdmin')->where('is_active', 1)->first();
            
            }else{

                $email_to = User::where('department', $ticket_from)->where('role', 'HOD')->where('is_active', 1)->first();

            }

            $mail_details['content'] = 'You have a Ticket - <b>'.$title.'</b> to approve by <b>'.$user->name.'</b>';
            $mail_details['url'] = URL::to('admin/unapproved/tickets/');
            
            $mail_details['title'] = 'Ticket Needs Approval';
            
            $subject = 'Ticket - '.$title.' by '.$user->name.' requires approval!';
    ...
    public function adminApproveTicket($id){
        ...
        if($ticket->ticket_from == Auth::user()->department){
            ...
            if($ticket->ticket_by != Auth::id()){
                ...
                $email_to = User::FindorFail($ticket->ticket_by);

                $mail_details['content'] = 'Your Ticket - <b>'.$ticket->title.'</b> has been approved';
                $mail_details['url'] = URL::to('staff/view/ticket/'.$ticket->id);
                
                $mail_details['title'] = 'Ticket Approved';
                
                $subject = 'Ticket - '.$ticket->title.' from '.$ticket->ticket_from.' Approved!';
            ...
            $email_to = User::where('department', $ticket->ticket_to)->where('role', 'HOD')->where('is_active', 1)->first();

            $mail_details['content'] = 'You have received a Ticket - <b>'.$ticket->title.'</b> from <b>'.$ticket->ticket_from.'</b>';
            $mail_details['url'] = URL::to('admin/view/ticket/'.$ticket->id);
            
            $mail_details['title'] = 'Ticket Received';
            
            $subject = 'Ticket - '.$ticket->title.' from '.$ticket->ticket_from.' Received!';
    ...
    public function respondTicket(Request $request){
        ...
        if(Auth::id() == $ticket->ticket_by){
            ...
            $email_to = User::findOrFail($ticket->assigned_to);

            $mail_details['content'] = 'Ticket Owner replied to you for the ticket - <b>'.$ticket->title.'</b> from <b>'.$ticket->ticket_from.'</b>';
            if($email_to->role == 'HOD'){
                $mail_details['url'] = URL::to('admin/view/ticket/'.$ticket_id);
            }elseif($email_to->role == 'SuperAdmin'){
                $mail_details['url'] = URL::to('superadmin/view/ticket/'.$ticket_id);
            }else{
                $mail_details['url'] = URL::to('staff/view/ticket/'.$ticket_id);
            }
            
            $mail_details['title'] = 'Ticket Owner Replied';
            
            $subject = 'Ticket - '.$ticket->title."'s Owner replied!";
        ...
        if(Auth::id() == $ticket->assigned_to){
            ...
            $mail_details['content'] = 'Staff responded to your Ticket - <b>'.$ticket->title.'</b> from <b>'.$ticket->ticket_from.'</b>';
            if($email_to->role == 'HOD'){
                $mail_details['url'] = URL::to('admin/view/ticket/'.$ticket_id);
            }elseif($email_to->role == 'SuperAdmin'){
                $mail_details['url'] = URL::to('superadmin/view/ticket/'.$ticket_id);
            ...
            $mail_details['title'] = 'Staff Responded';
            
            $subject = 'Ticket - '.$ticket->title."'s Staff Response!";
    ...
    public function assignTicket($emp_id, $ticket_id){
        ...
        if($ticket->assigned_to == Auth::id()){
            ...
            $mail_details['content'] = 'Ticket - <b>'.$ticket->title.'</b> from <b>'.$ticket->ticket_from.'</b> has been assgined to you';
            if($email_to->role == 'HOD'){
                $mail_details['url'] = URL::to('admin/view/ticket/'.$ticket_id);
            }elseif($email_to->role == 'SuperAdmin'){
                $mail_details['url'] = URL::to('superadmin/view/ticket/'.$ticket_id);
            ...
            $mail_details['title'] = 'Ticket Assigned';

            $subject = 'Ticket - '.$ticket->title.' from '.$ticket->ticket_from.' Assigned to you!';
        ...
        $mail_details['content'] = 'Staff has been assigned to your Ticket - <b>'.$ticket->title.'</b>';
        if($email_to->role == 'HOD'){
            $mail_details['url'] = URL::to('admin/view/ticket/'.$ticket_id);
        }elseif($email_to->role == 'SuperAdmin'){
            $mail_details['url'] = URL::to('superadmin/view/ticket/'.$ticket_id);
        ...
        $mail_details['title'] = 'Staff Assigned!';

        $subject = 'Staff Assigned to your Ticket - '.$ticket->title;
    ...
    public function changeTicketStatus($ticket_status, $ticket_id){
        ...
        if($ticket_status == 'Reopen'){
            ...
            $email_to = User::findOrFail($ticket->assigned_by);

            $mail_details['content'] = 'Ticket - <b>'.$ticket->title.'</b> from <b>'.$ticket->ticket_from.'</b> has been Reopened';
            if($email_to->role == 'HOD'){
                $mail_details['url'] = URL::to('admin/view/ticket/'.$ticket_id);
            }elseif($email_to->role == 'SuperAdmin'){
                $mail_details['url'] = URL::to('superadmin/view/ticket/'.$ticket_id);
            ...
            $mail_details['title'] = 'Ticket Reopened';

            $subject = 'Ticket - '.$ticket->title.' from '.$ticket->ticket_from.' has been re-opened!';
            ...
            $mail_details['content'] = 'Ticket - <b>'.$ticket->title.'</b> from <b>'.$ticket->ticket_from.'</b> is in progress';
            if($email_to->role == 'HOD'){
                $mail_details['url'] = URL::to('admin/view/ticket/'.$ticket_id);
            }elseif($email_to->role == 'SuperAdmin'){
                $mail_details['url'] = URL::to('superadmin/view/ticket/'.$ticket_id);
            ...
            $mail_details['title'] = 'Ticket is INPROCESS';

            $subject = 'Ticket - '.$ticket->title.' from '.$ticket->ticket_from.' is in progress';
            ...
            $mail_details['content'] = 'Your Ticket - <b>'.$ticket->title.'</b> from <b>'.$ticket->ticket_from.'</b> is in progress';
            if($email_to->role == 'HOD'){
                $mail_details['url'] = URL::to('admin/view/ticket/'.$ticket_id);
            }elseif($email_to->role == 'SuperAdmin'){
                $mail_details['url'] = URL::to('superadmin/view/ticket/'.$ticket_id);
            ...
            $subject = 'Your Ticket - '.$ticket->title.' from '.$ticket->ticket_from.' is in progress';
            ...
            $mail_details['content'] = 'Ticket - <b>'.$ticket->title.'</b> from <b>'.$ticket->ticket_from.'</b> is put on hold';
            if($email_to->role == 'HOD'){
                $mail_details['url'] = URL::to('admin/view/ticket/'.$ticket_id);
            }elseif($email_to->role == 'SuperAdmin'){
                $mail_details['url'] = URL::to('superadmin/view/ticket/'.$ticket_id);
            ...
            $mail_details['title'] = 'Ticket is on Hold';

            $subject = 'Ticket - '.$ticket->title.' from '.$ticket->ticket_from.' is put on hold';
            ...
            $mail_details['content'] = 'Your Ticket - <b>'.$ticket->title.'</b> from <b>'.$ticket->ticket_from.'</b> is put on hold';
            if($email_to->role == 'HOD'){
                $mail_details['url'] = URL::to('admin/view/ticket/'.$ticket_id);
            }elseif($email_to->role == 'SuperAdmin'){
                $mail_details['url'] = URL::to('superadmin/view/ticket/'.$ticket_id);
            ...
            $subject = 'Your Ticket - '.$ticket->title.' from '.$ticket->ticket_from.' is put on hold';
            ...
            $mail_details['title'] = 'Ticket Completed';

            $subject = 'Ticket - '.$ticket->title.' from '.$ticket->ticket_from.' has been marked as completed';
            ...
            $mail_details['content'] = 'Your Ticket - <b>'.$ticket->title.'</b> from <b>'.$ticket->ticket_from.'</b> has been completed. Please visit the application and close the ticket';
            if($email_to->role == 'HOD'){
                $mail_details['url'] = URL::to('admin/view/ticket/'.$ticket_id);
            }elseif($email_to->role == 'SuperAdmin'){
                $mail_details['url'] = URL::to('superadmin/view/ticket/'.$ticket_id);
            ...
            $subject = 'Your Ticket - '.$ticket->title.' from '.$ticket->ticket_from.' has been completed';
    ...
    public function changeTicketPriority($ticket_id, $priority){
        ...
        $mail_details['content'] = 'Your Ticket - <b>'.$ticket->title.' priority has been changed';
        if($email_to->role == 'HOD'){
            $mail_details['url'] = URL::to('admin/view/ticket/'.$ticket_id);
        }elseif($email_to->role == 'SuperAdmin'){
            $mail_details['url'] = URL::to('superadmin/view/ticket/'.$ticket_id);
        ...
        $mail_details['title'] = 'Ticket Priority Changed';

        $subject = 'Ticket - '.$ticket->title.'priority has been Changed';
        ...
        if(Auth::user()->department != $ticket->ticket_from){
            ...
            $mail_details['content'] = 'Ticket - <b>'.$ticket->title.' priority has been changed';
            if($email_to->role == 'HOD'){
                $mail_details['url'] = URL::to('admin/view/ticket/'.$ticket_id);
            }elseif($email_to->role == 'SuperAdmin'){
                $mail_details['url'] = URL::to('superadmin/view/ticket/'.$ticket_id);
    ...
    public function closeTicket(Request $request){
        ...
        $mail_details['content'] = 'Ticket - <b>'.$ticket->title.'</b> from <b>'.$ticket->ticket_from.'</b> has been Closed';
        if($email_to->role == 'HOD'){
            $mail_details['url'] = URL::to('admin/view/ticket/'.$ticket_id);
        }elseif($email_to->role == 'SuperAdmin'){
            $mail_details['url'] = URL::to('superadmin/view/ticket/'.$ticket_id);
        ...
        $mail_details['title'] = 'Ticket Closed';

        $subject = 'Ticket - '.$ticket->title.' from '.$ticket->ticket_from.' has been Closed';
